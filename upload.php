...
if(empty($user) || empty($group) || !$admin) {
	...
	if($form_type == "workout") {
		...
		$text = addslashes($_POST['ptext']);
		$error = "";

		if($_FILES['ppdf']['error'][0] == 4) {
			$file_count = 0;
		}
		else {
			$file_count = count($_FILES['ppdf']['name']);
		}

		$file_names = array();
		$files = ($file_count>0)?reArrayFiles($_FILES['ppdf']):array();
		$uploaddir = "plays";
		...
		// Otherwise, we were successful, so let's add to the db!
			...
			if(empty($title) || empty($text)) {
				$error = "The play must have a title and a body!";
			}
			else {
				mysql_query("INSERT INTO plays (g_id, title, text) VALUES ('" . $group['id'] . "','$title','$text')");
				$p_id = mysql_insert_id();
				$i = 0;
				foreach ($file_names as $name) {
					mysql_query("INSERT INTO play_files (p_id, name, filepath) VALUE ('$p_id', '" . substr($files[$i++]['name'], 0, -4) . "', '$name')");
				}
				$error = "Play successfully posted!";
			}
		}
